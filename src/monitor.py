import psutil, config, time, logging, tarfile, os.path, sys, os
    ...
    while True:
        try:
            if (get_ram_percent() >= config.MAX_MEMORY):
                ...
                #in case the memory is heigher for more then 15 seconds we wat to increase the log time to not spam it with the same data
                ...
                if (OCCURENCE >= max_occurence):
                    for p in get_procs_by_usage():
                        if p['memory_percent'] > config.MIN_MEMORY:
                            logging.warning("The process {0} with PID: {1} uses {2}% memory, is executed by {3} with the initial command: {4} \nMore info at /opt/resmon/archives/{5}.tar.gz".format(p['name'], p['pid'], p['memory_percent'], p['username'], ' '.join(psutil.Process(p['pid']).cmdline()), p['pid']))
